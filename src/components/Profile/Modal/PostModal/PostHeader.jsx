...
function PostHeader(props) {
    ...
    const { username, pfp, caption, createdBy,createdAt, id, handleCloseModal } = props;
    ...
    async function handleDeletePost() {
        ...
        setIsDeleting(true);
        try {
            const { data } = await supabase.storage.from('images').list(`postPics/${id}`);
            if (data && data.length > 0) {
                const paths = data.map(file => `postPics/${id}/${file.name}`);
