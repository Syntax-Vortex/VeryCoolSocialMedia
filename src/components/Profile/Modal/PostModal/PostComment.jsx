...
function PostComment(props) {
    ...
    return (
        <div className="flex-1 flex flex-col lg:flex-row gap-2 justify-start items-start w-full">
            <ErrorPopup />
            <div className="flex gap-2 justify-start items-start">
                ...
                <button className={`hover:text-red-700 duration-100 ${ownComment ? 'block lg:hidden' : 'hidden'} mt-1 ml-auto`} onClick={handleDeleteComment} disabled={isDeleting}>
            ...
            <button className={`hover:text-red-700 duration-100 ${ownComment ? 'hidden lg:block' : 'hidden'} mt-1 ml-auto`} onClick={handleDeleteComment} disabled={isDeleting}>
                {isDeleting ? (<MinimalLoader />) : (<svg className="size-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10 11v6" /><path d="M14 11v6" /><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6" /><path d="M3 6h18" /><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2" /></svg>)}
            </button>
        </div>
    )
